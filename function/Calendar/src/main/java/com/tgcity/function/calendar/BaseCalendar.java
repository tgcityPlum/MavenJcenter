...
public abstract class BaseCalendar extends ViewPager implements ICalendar {
    ...
    private void initAdapter() {
        ...
        if (mStartDate.isAfter(mEndDate)) {
            throw new IllegalArgumentException(getContext().getString(R.string.nc_start_after_end));
        }

        if (mStartDate.isBefore(new LocalDate(mDefaultStartDate))) {
            throw new IllegalArgumentException(getContext().getString(R.string.nc_start_before_19010101));
        }

        if (mEndDate.isAfter(new LocalDate(mDefaultEndDateDate))) {
            throw new IllegalArgumentException(getContext().getString(R.string.nc_end_after_20991231));
        }

        if (mStartDate.isAfter(mInitializeDate) || mEndDate.isBefore(mInitializeDate)) {
            throw new IllegalArgumentException(getContext().getString(R.string.nc_initialize_date_illegal));
        }
    ...
    public void setDateInterval(String startFormatDate, String endFormatDate) {
        try {
            ...
            throw new IllegalArgumentException(getContext().getString(R.string.nc_date_format_illegal));
        }
        initAdapter();
    }
    ...
    public void onClickCurrentMonthOrWeekDate(LocalDate localDate) {
        if (mOnCalendarChangedListener != null){
            mOnCalendarChangedListener.onCalendarClick(localDate);
        }
//        jump(localDate, true, DateChangeBehavior.CLICK);
        ...
        if (!isAvailable(localDate)) {
            if (getVisibility() == VISIBLE) {
                if (mOnClickDisableDateListener != null) {
                    ...
                    Toast.makeText(getContext(), TextUtils.isEmpty(mAttrs.disabledString) ? getResources().getString(R.string.nc_disabledString) : mAttrs.disabledString, Toast.LENGTH_SHORT).show();
                }
            }
        } else {
    ...
    public void jumpMonth(int year, int month) {
        ...
        try {
            ...
            throw new IllegalArgumentException(getContext().getString(R.string.nc_date_format_jump));
        }
    ...
    public void setCheckedDates(List<String> dateList) {
        ...
        if (mCheckModel != CheckModel.MULTIPLE) {
            throw new RuntimeException(getContext().getString(R.string.nc_set_checked_dates_illegal));
        }

        if (mMultipleCountModel != null && dateList.size() > mMultipleCount) {
            throw new RuntimeException(getContext().getString(R.string.nc_set_checked_dates_count_illegal));
        }
        mTotalCheckedDateList.clear();
        try {
