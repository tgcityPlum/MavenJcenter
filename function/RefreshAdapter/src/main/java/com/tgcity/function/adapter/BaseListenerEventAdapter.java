...
import com.tgcity.function.adapter.eventCallBack.OnCheckedChangeForRadioButtonOrCheckBoxCallBack;
import com.tgcity.function.adapter.eventCallBack.OnCheckedChangeForRadioGroupCallBack;
import com.tgcity.function.adapter.eventCallBack.OnClickCallBack;
import com.tgcity.function.adapter.eventCallBack.OnTouchCallBack;
import com.tgcity.utils.check.ContinuousClick;
        ...
        if (helper.getTag(viewId) == null || !(helper.getTag(viewId) instanceof OnClickCallBack)) {
            OnClickCallBack onClickCallBack = new OnClickCallBack(viewId, helper, item, new OnClickCallBack.OnClick<T, K>() {
                ...
                public void onClick(int viewId, K helper, T item) {
                    if (ContinuousClick.check400(viewId)) {
                        return;
                    }
