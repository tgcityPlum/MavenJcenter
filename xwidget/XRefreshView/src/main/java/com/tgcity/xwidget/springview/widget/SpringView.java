package com.tgcity.xwidget.springview.widget;
...
public class SpringView extends ViewGroup {
    ...
    //当出现刷新出错时手指移动此距离触发布局重置
    private double EXCEPTION_REFRESH = 20;
    ...
    public void clear() {
        ...
        EXCEPTION_REFRESH = 0;
        MOVE_PARA = 0;
        MAX_HEADER_PULL_HEIGHT = 0;
        MAX_FOOTER_PULL_HEIGHT = 0;
        HEADER_LIMIT_HEIGHT = 0;
        FOOTER_LIMIT_HEIGHT = 0;
        HEADER_SPRING_HEIGHT = 0;
        FOOTER_SPRING_HEIGHT = 0;
        mLastY = 0;
        mLastX = 0;
        mfirstY = 0;
        isInControl = false;
        ...
        contentView = null;
        headerResoureId = 0;
        int footerResoureId = 0;
        appbarState = null;
        appBarCouldScroll = false;
    ...
    public SpringView(Context context, AttributeSet attrs) {
        ...
        //获取自定义属性
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.xrv_SpringView);
        if (ta.hasValue(R.styleable.xrv_SpringView_xrv_type)) {
            int type_int = ta.getInt(R.styleable.xrv_SpringView_xrv_type, 0);
            type = Type.values()[type_int];
        }
        if (ta.hasValue(R.styleable.xrv_SpringView_xrv_give)) {
            int give_int = ta.getInt(R.styleable.xrv_SpringView_xrv_give, 0);
            give = Give.values()[give_int];
        }
        if (ta.hasValue(R.styleable.xrv_SpringView_xrv_header)) {
            headerResoureId = ta.getResourceId(R.styleable.xrv_SpringView_xrv_header, 0);
        }
        if (ta.hasValue(R.styleable.xrv_SpringView_xrv_footer)) {
            footerResoureId = ta.getResourceId(R.styleable.xrv_SpringView_xrv_footer, 0);
        }
        ta.recycle();
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        return isInterceptEventFromUser && isNeedMyMove;
    }
    ...
    private void callOnDropAnim() {
        if (getScrollY() < 0 && headerHander != null) {
            headerHander.onDropAnim(header, -getScrollY());
        }
        if (getScrollY() > 0 && footerHander != null) {
    ...
    private boolean isInterceptEventFromUser = true;

    /**
     * 用户主动设置是否拦截事件
     */
    public void setInterceptEvent(boolean isIntercept) {
        isInterceptEventFromUser = isIntercept;
    }
    ...
    public interface OnFreshListener {
         ...
         * 准备刷新或加载
         *
         * @param HeaderOrFooter true：header ,false:footer
         */

        void onPreDrag(boolean HeaderOrFooter);
...
//        contentLay.bringToFront(); //把内容放在最前端
    ...
    public void setResult(ErrorMode o) {
        ...
        if (headerHander != null) {
            headerHander.result(o);
        }
        if (_footerHander != null) {
            _footerHander.result(o);
        }
        if (footerHander != null) {
            footerHander.result(o);
        }
    }
     ...
     * @param o
     */
    public void setErrorImg(int o) {
        if (_headerHander != null) {
            _headerHander.setErrorImg(o);
        }
        if (headerHander != null) {
            headerHander.setErrorImg(o);
        }
        if (_footerHander != null) {
            _footerHander.setErrorImg(o);
        }
        if (footerHander != null) {
            footerHander.setErrorImg(o);
        }
    }
    ...
    public void setErrorTitleAndDesc(String title, String desc) {
        if (_headerHander != null) {
            _headerHander.setErrorTitleAndDesc(title, desc);
        }
        if (headerHander != null) {
            headerHander.setErrorTitleAndDesc(title, desc);
        }
        if (_footerHander != null) {
            _footerHander.setErrorTitleAndDesc(title, desc);
        }
        if (footerHander != null) {
            footerHander.setErrorTitleAndDesc(title, desc);
        }
    }
     ...
     */
    public void setErrorClick(OnExceptionEventClickListener onExceptionEventClickListener) {
        if (_headerHander != null) {
            _headerHander.setOnExceptionEventClickListener(onExceptionEventClickListener);
        }
        if (headerHander != null) {
            headerHander.setOnExceptionEventClickListener(onExceptionEventClickListener);
        }
        if (_footerHander != null) {
            _footerHander.setOnExceptionEventClickListener(onExceptionEventClickListener);
        }
        if (footerHander != null) {
            footerHander.setOnExceptionEventClickListener(onExceptionEventClickListener);
        }
    }

    public interface DragHander {
        void onPreMove(View rootView);
        ...
        void onFingerRemoval(View rootView);
         ...
         * 发生异常时的点击事件回调
         */
        DragHander setOnExceptionEventClickListener(OnExceptionEventClickListener onExceptionEventClickListener);
         ...
         * @param desc
         */
        void setErrorTitleAndDesc(String title, String desc);
     ...
     * 发生异常时的点击事件回调
     */
    public interface OnExceptionEventClickListener {
        void onExceptionClick(String errorMode, Context context);
    }
}
