 ...
 * Example:
 * <pre><code>
 *     //请先初始化LogUtils，LogUtils会帮您只在debug环境下输出日志
 *     LogUtils.init();
 *     ```
 *     LogUtils.e("string");
 *
...
public class LogUtils {
    ...
    private static void log(final int type, boolean showLeftBorder, final String tag, final Object... contents) {
        ...
        if (CONFIG.mLog2ConsoleSwitch && type_low >= CONFIG.mConsoleFilter && type_high != FILE) {
            print2Console(type_low, showLeftBorder, tagHead.tag, tagHead.consoleHead, body);
        }
        if ((CONFIG.mLog2FileSwitch || type_high == FILE) && type_low >= CONFIG.mFileFilter) {
            print2File(type_low, showLeftBorder, tagHead.tag, tagHead.fileHead + body);
        }
    }
    ...
    private static void print2Console(final int type, boolean showLeftBorder,
                                      final String tag,
                                      final String[] head,
                                      final String msg) {
        if (CONFIG.mSingleTagSwitch) {
            ...
            if (CONFIG.mLogBorderSwitch) {
                ...
                if (head != null) {
                    for (String aHead : head) {
                        sb.append(showLeftBorder ? LEFT_BORDER : "").append(aHead).append(LINE_SEP);
                    }
                    sb.append(MIDDLE_BORDER).append(LINE_SEP);
                }
                for (String line : msg.split(LINE_SEP)) {
                    sb.append(showLeftBorder ? LEFT_BORDER : "").append(line).append(LINE_SEP);
                }
            ...
            printMsgSingleTag(type, showLeftBorder, tag, sb.toString());
        } else {
            printBorder(type, tag, true);
            printHead(type, showLeftBorder, tag, head);
            printMsg(type, showLeftBorder, tag, msg);
    ...
    private static void printHead(final int type, boolean showLeftBorder, final String tag, final String[] head) {
        if (head != null) {
            for (String aHead : head) {
    ...
    private static void printMsg(final int type, boolean showLeftBorder, final String tag, final String msg) {
        int len = msg.length();
        int countOfSub = len / MAX_LEN;
        if (countOfSub > 0) {
            int index = 0;
            for (int i = 0; i < countOfSub; i++) {
                printSubMsg(type, showLeftBorder, tag, msg.substring(index, index + MAX_LEN));
                index += MAX_LEN;
            }
            if (index != len) {
                printSubMsg(type, showLeftBorder, tag, msg.substring(index, len));
            }
        } else {
            printSubMsg(type, showLeftBorder, tag, msg);
        }
    }

    private static void printMsgSingleTag(final int type, boolean showLeftBorder, final String tag, final String msg) {
        int len = msg.length();
        int countOfSub = len / MAX_LEN;
        if (countOfSub > 0) {
            if (CONFIG.mLogBorderSwitch) {
                ...
                for (int i = 1; i < countOfSub; i++) {
                    Log.println(type, tag, PLACEHOLDER + LINE_SEP + TOP_BORDER + LINE_SEP
                            + (showLeftBorder ? LEFT_BORDER : "") + msg.substring(index, index + MAX_LEN)
                            + LINE_SEP + BOTTOM_BORDER);
                    index += MAX_LEN;
                }
                if (index != len) {
                    Log.println(type, tag, PLACEHOLDER + LINE_SEP + TOP_BORDER + LINE_SEP
                            + (showLeftBorder ? LEFT_BORDER : "") + msg.substring(index, len));
                }
            } else {
                int index = 0;
                for (int i = 0; i < countOfSub; i++) {
    ...
    private static void printSubMsg(final int type, boolean showLeftBorder, final String tag, final String msg) {
        if (!CONFIG.mLogBorderSwitch) {
            Log.println(type, tag, msg);
            return;
        }
        ...
        for (String line : lines) {
            Log.println(type, tag, showLeftBorder ? LEFT_BORDER : "" + line);
        }
    }
    ...
    private static void print2File(final int type, boolean showLeftBorder, final String tag, final String msg) {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = MyApplication.getInstances()
                    .getPackageManager()
                    .getPackageInfo(MyApplication.getInstances().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        ...
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && MyApplication.getInstances().getApplicationContext().getExternalCacheDir() != null)
                mDefaultDir = MyApplication.getInstances().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                mDefaultDir = MyApplication.getInstances().getApplicationContext().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }*/
        }
